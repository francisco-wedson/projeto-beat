...
class Player_select():
    def __init__(self, screen, background):
        ...
        self.hitboxes = [self.hitbox_solo, self.hitbox_vs]
        ...
        self.text_x_normal = self.font_x.render("X", True, self.color_normal)
        self.text_x_selected = self.font_x.render("X", True, self.color_selected)
        self.text_x_rect = self.text_x_normal.get_rect(center=(1260, 580))
    ...
    def run(self, events, dt):
        ...
        for event in events:
            ...
            if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                if self.hitboxes[self.selected_index].collidepoint(self.mouse_pos):
                    player_mode = self.options[self.selected_index]
                    if player_mode == "solo": return 'char_select', 1
                    if player_mode == "versus": return 'char_select', 2
        ...
        if self.selected_index == 1:
            ...
            self.screen.blit(self.text_x_selected, self.text_x_rect)

        else:
            self.screen.blit(self.text_vs_normal, self.text_vs_rect)
            self.screen.blit(self.text_x_normal, self.text_x_rect)
